...
const ScoreRing = ({ score }) => (
  <div className="relative w-12 h-12 flex-shrink-0">
    <svg className="w-12 h-12 -rotate-90" viewBox="0 0 40 40">
      <circle cx="20" cy="20" r="16" fill="none" stroke="rgb(var(--ink-border))" strokeWidth="3" />
      {score !== null && (
        <circle cx="20" cy="20" r="16" fill="none" stroke="rgb(var(--gold))" strokeWidth="3"
          strokeDasharray={`${(score || 0) * 1.005} 100.5`} strokeLinecap="round" />
      )}
    </svg>
    <div className="absolute inset-0 flex items-center justify-center">
      <span className="text-[11px] font-semibold text-[rgb(var(--text-main))]">{score === null ? '—' : score}</span>
    </div>
  </div>
);
    ...
    <div className="space-y-6 animate-fade-in-up" data-testid="community-dashboard-page">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <UsersThree size={20} weight="duotone" className="text-[rgb(var(--gold))]" />
        ...
        <select
          ...
          className="px-3 py-2 rounded-md bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] text-xs text-[rgb(var(--text-main))] focus:outline-none focus:border-[rgb(var(--gold)/0.5)]"
      ...
      {parentVillage && (
        <button onClick={() => setScope(`village:${parentVillage.id}`)}
          className="flex items-center gap-1.5 text-xs text-[rgb(var(--text-muted))] hover:text-[rgb(var(--gold))]" data-testid="back-to-village">
      ...
      {loading && <p className="text-sm text-[rgb(var(--text-muted))]">Listening to the village…</p>}

      {data && data.dimensions && (
        <>
          {/* May need you this week */}
          {data.attention?.length > 0 && (
            <Card className="bg-[rgb(var(--ink-card))] border-amber-500/30">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {data.attention.map(a => (
                    <span key={a.id} className="flex items-center gap-1.5 text-xs text-[rgb(var(--text-main))] bg-[rgb(var(--ink-deep))] border border-[rgb(var(--ink-border))] rounded-full px-2.5 py-1.5">
                ...
                <p className="text-[10px] text-[rgb(var(--text-muted))] mt-2 italic">
                ...
                {data.village_mentors?.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[rgb(var(--ink-border))]">
                    <p className="text-[10px] tracking-[0.15em] uppercase text-[rgb(var(--gold))] mb-1.5 flex items-center gap-1">
                      <HandHeart size={11} weight="duotone" /> Suggest a mentor check-in
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {data.village_mentors.map(m => (
                        <span key={m.id} className="flex items-center gap-1.5 text-xs text-[rgb(var(--text-main))] bg-[rgb(var(--ink-deep))] border border-[rgb(var(--gold)/0.2)] rounded-full px-2.5 py-1">
                    ...
                    <p className="text-[10px] text-[rgb(var(--text-faint))] mt-1.5 italic">
          ...
          {data.cohorts?.length > 0 && (
            <div className="flex flex-wrap items-center gap-2" data-testid="cohort-drilldown">
              <span className="text-[10px] tracking-[0.15em] uppercase text-[rgb(var(--text-faint))]">Drill into a cohort:</span>
              {data.cohorts.map(c => (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {DIMENSIONS.map(d => {
              ...
              return (
                <Card key={d.key} className="bg-[rgb(var(--ink-card))] border-[rgb(var(--ink-border))]">
                  <CardContent className="p-4 flex items-center gap-3">
                    <ScoreRing score={dim?.score ?? null} />
                    <div className="min-w-0">
                      <p className="text-sm text-[rgb(var(--text-main))] flex items-center gap-1.5">
                        <d.icon size={14} weight="duotone" className="text-[rgb(var(--gold))]" /> {d.label}
                      </p>
                      <p className="text-[10px] text-[rgb(var(--text-muted))] leading-snug mt-0.5">{dim?.note || d.blurb}</p>
                    </div>
                  </CardContent>
                </Card>
              );
          ...
          <Card className="bg-[rgb(var(--ink-card))] border-[rgb(var(--ink-border))]">
            ...
            <CardContent className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[9px] uppercase tracking-wider text-[rgb(var(--text-muted))]">
                ...
                <tbody>
                  {data.members.map(m => (
                    <tr key={m.id} className="border-t border-[rgb(var(--ink-border))]" data-testid={`dash-member-${m.id}`}>
                      <td className="py-2 pr-3">
                        <span className="flex items-center gap-2 text-xs text-[rgb(var(--text-main))]">
                          ...
                          {m.name}
                          {m.is_mentor && <HandHeart size={12} className="text-[rgb(var(--gold))]" title="Mentor" />}
                        </span>
                      </td>
                      <td className="py-2 px-2 text-center text-xs text-[rgb(var(--text-muted))]">{m.active_days}</td>
                      {['belonging', 'participation', 'contribution', 'collaboration', 'leadership', 'wellness'].map(k => {
                        const s = m.scores[k];
                        return (
                          <td key={k} className="py-2 px-2">
                            {s === null || s === undefined ? (
                              <p className="text-center text-[10px] text-[rgb(var(--text-faint))]">—</p>
                            ) : (
                              <div className="w-14 h-1.5 bg-[rgb(var(--ink-secondary))] rounded-full overflow-hidden mx-auto">
                                <div className="h-full bg-[rgb(var(--gold))] rounded-full" style={{ width: `${Math.max(s, 2)}%` }} />
                              </div>
                            )}
                          </td>
                        );
              ...
              <p className="text-[10px] text-[rgb(var(--text-faint))] mt-3 italic">
      ...
      {data && !data.dimensions && (
        <p className="text-sm text-[rgb(var(--text-muted))]">{data.message}</p>
      )}
    </div>
  );
}
