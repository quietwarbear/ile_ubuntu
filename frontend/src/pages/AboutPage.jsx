...
const FEATURES = [
  ...
  { icon: CreditCard, title: 'Membership Tiers', desc: 'Stripe-powered subscriptions: Explorer (free), Scholar ($19.99), Elder Circle ($49.99). Feature gating enforced platform-wide.', color: 'text-[rgb(var(--gold))]', bg: 'bg-[rgb(var(--gold)/0.1)] border-[rgb(var(--gold)/0.2)]' },
...
const TIERS = [
  ...
  { id: 'scholar', name: 'Scholar', price: '$19.99/mo', icon: Crown, color: 'text-[rgb(var(--gold))]', border: 'border-[rgb(var(--gold)/0.3)]', features: ['All Explorer features', 'Unlimited enrollment', 'Cohort membership', 'Knowledge spaces', 'Priority support'] },
...
export default function AboutPage() {
  ...
  return (
    <div className="theme-midnight min-h-screen bg-[rgb(var(--ink-deep))]" data-testid="about-page">
      {/* Hero */}
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04]" style={{
          backgroundImage: 'radial-gradient(circle at 30% 40%, rgb(var(--gold)) 1px, transparent 1px), radial-gradient(circle at 70% 60%, rgb(var(--gold)) 1px, transparent 1px)',
          backgroundSize: '60px 60px',
        }} />
        <div className="max-w-4xl mx-auto px-6 py-20 text-center relative">
          <div className="w-20 h-20 rounded-full bg-[rgb(var(--gold)/0.1)] border border-[rgb(var(--gold)/0.3)] flex items-center justify-center mx-auto mb-6">
            <BrandMark className="w-12 h-12 object-contain" />
          </div>
          <h1 className="text-4xl sm:text-5xl text-[rgb(var(--text-main))] mb-3" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            The Ile Ubuntu
          </h1>
          <p className="text-base text-[rgb(var(--gold))] mb-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            {t('about_hero')}
          </p>
          <p className="text-sm text-[rgb(var(--text-muted))] max-w-xl mx-auto mb-6">
            {t('about_subtitle')}
          </p>
          <p className="text-xs text-[rgb(var(--text-faint))] max-w-md mx-auto">
            {t('about_tagline')}
          </p>
          <button
            onClick={() => navigate('/dashboard')}
            className="mt-8 px-6 py-2.5 bg-[rgb(var(--gold))] text-[rgb(var(--ink-deep))] rounded-md text-sm font-medium hover:bg-[rgb(var(--gold)/0.9)] transition-all inline-flex items-center gap-2"
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        <h2 className="text-2xl text-[rgb(var(--text-main))] text-center mb-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
          Platform Features
        </h2>
        <p className="text-xs text-[rgb(var(--text-muted))] text-center mb-10">Everything you need for a thriving learning community</p>
          ...
          {FEATURES.map((f) => {
            ...
            return (
              <div key={f.title} className={`p-4 rounded-lg border ${f.bg} transition-all hover:-translate-y-0.5`} data-testid={`feature-${f.title.toLowerCase().replace(/\s+/g, '-')}`}>
                ...
                <h3 className="text-sm text-[rgb(var(--text-main))] font-medium mb-1">{f.title}</h3>
                <p className="text-[10px] text-[rgb(var(--text-muted))] leading-relaxed">{f.desc}</p>
              </div>
            );
          })}
        </div>
      ...
      {/* Membership Tiers */}
      <div className="bg-[rgb(var(--ink-secondary))] py-16">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-2xl text-[rgb(var(--text-main))] text-center mb-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            Membership Tiers
          </h2>
          <p className="text-xs text-[rgb(var(--text-muted))] text-center mb-8">Choose your path</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4" data-testid="tiers-section">
            {TIERS.map((tier) => {
              const Icon = tier.icon;
              return (
                <div key={tier.id} className={`p-5 rounded-lg border ${tier.border} bg-[rgb(var(--ink-card))]`} data-testid={`about-tier-${tier.id}`}>
                  ...
                  <p className="text-xl font-bold text-[rgb(var(--text-main))] mt-1">{tier.price}</p>
                  <ul className="mt-3 space-y-1.5">
                    {tier.features.map((feat, i) => (
                      <li key={i} className="flex items-start gap-2 text-[10px] text-[rgb(var(--text-muted))]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <p className="text-xs text-[rgb(var(--text-muted))] text-center mb-8">Differentiated governance for the learning commons</p>
        <div className="flex flex-wrap justify-center gap-3" data-testid="roles-section">
          {ROLES.map((r) => (
            <div key={r.name} className="p-3 rounded-lg bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] text-center min-w-[120px]">
              <p className="text-sm text-[rgb(var(--gold))] font-medium">{r.name}</p>
              <p className="text-[9px] text-[rgb(var(--text-muted))] mt-1">{r.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Tech Stack */}
      <div className="bg-[rgb(var(--ink-secondary))] py-12">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-lg text-[rgb(var(--text-main))] mb-4" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            Built With
          </h2>
          <div className="flex flex-wrap justify-center gap-2 text-[10px] text-[rgb(var(--text-muted))]">
            {['React', 'FastAPI', 'MongoDB', 'Tailwind CSS', 'Jitsi Meet', 'Stripe', 'Resend', 'Google APIs', 'PWA'].map(tech => (
              <span key={tech} className="px-3 py-1 bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] rounded-full">{tech}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16" data-testid="social-strategy">
        ...
        <p className="text-xs text-[rgb(var(--text-muted))] text-center mb-8">Copy-ready content for your social channels</p>

        <div className="space-y-4">
          {/* Instagram/Twitter */}
          <div className="p-4 bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] rounded-lg">
            ...
            <p className="text-sm text-[rgb(var(--text-main))] leading-relaxed mb-2">
            ...
            <p className="text-xs text-[rgb(var(--gold))]">
          ...
          <div className="p-4 bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] rounded-lg">
            ...
            <p className="text-sm text-[rgb(var(--text-muted))] leading-relaxed mb-2">
          ...
          {/* One-liner */}
          <div className="p-4 bg-[rgb(var(--ink-card))] border border-[rgb(var(--gold)/0.2)] rounded-lg text-center">
            <p className="text-[9px] text-[rgb(var(--gold))] uppercase tracking-wider mb-2">Elevator Pitch</p>
            <p className="text-sm text-[rgb(var(--text-main))] italic">
      ...
      {/* Footer CTA */}
      <div className="py-12 text-center border-t border-[rgb(var(--ink-border))]">
        <button
          onClick={() => navigate('/dashboard')}
          className="px-6 py-2.5 bg-[rgb(var(--gold))] text-[rgb(var(--ink-deep))] rounded-md text-sm font-medium hover:bg-[rgb(var(--gold)/0.9)] transition-all inline-flex items-center gap-2"
        >
          Start Learning <ArrowRight size={16} />
        </button>
        <p className="text-[10px] text-[rgb(var(--text-faint))] mt-3">The Ile Ubuntu &mdash; Living Learning Commons</p>
      </div>
    </div>
  );
}
