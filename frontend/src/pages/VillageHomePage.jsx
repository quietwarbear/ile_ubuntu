...
export default function VillageHomePage({ user }) {
  ...
  if (!home) return <p className="text-sm text-[rgb(var(--text-muted))]">{t('village_entering')}</p>;
    ...
    <div className="space-y-6 animate-fade-in-up max-w-4xl" data-testid="village-home">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          <TreeEvergreen size={24} weight="duotone" className="text-[rgb(var(--gold))]" />
          <h1 className="text-2xl text-[rgb(var(--text-main))]" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{village.name}</h1>
        </div>
        <div className="flex flex-wrap gap-3 text-xs text-[rgb(var(--text-muted))]">
          ...
          <button onClick={() => navigate(`/villages/${village.id}`)} className="flex items-center gap-0.5 text-[rgb(var(--gold))] hover:underline">
      ...
      <Card className="bg-gradient-to-r from-[rgb(var(--gold)/0.1)] to-transparent border-[rgb(var(--gold)/0.3)]">
        <CardContent className="p-4 flex items-start gap-3">
          <Sparkle size={18} weight="duotone" className="text-[rgb(var(--gold))] flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-[10px] tracking-[0.2em] uppercase text-[rgb(var(--gold))] mb-1">{t('village_elder_word')}</p>
            <p className="text-sm text-[rgb(var(--text-main))] italic" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{elder_prompt}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="bg-[rgb(var(--ink-card))] border-[rgb(var(--ink-border))]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-[rgb(var(--text-main))] flex items-center gap-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
              <VideoCamera size={15} weight="duotone" className="text-[rgb(var(--gold))]" /> {t('village_sessions')}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5">
            {sessions.map(s => (
              <button key={s.id} onClick={() => navigate(s.status === 'live' ? `/live/${s.id}` : '/live')}
                className="w-full flex items-center gap-2.5 p-2 rounded bg-[rgb(var(--ink-deep))] border border-[rgb(var(--ink-border))] text-left hover:border-[rgb(var(--gold)/0.25)]">
                <Avatar person={s} size="w-6 h-6" />
                <span className="flex-1 min-w-0">
                  <span className="block text-xs text-[rgb(var(--text-main))] truncate">{s.title}</span>
                  <span className="block text-[10px] text-[rgb(var(--text-muted))]">{s.host_name}{s.scheduled_at ? ` · ${fmtWhen(s.scheduled_at)}` : ''}</span>
                </span>
                {s.status === 'live' && (
            ...
            {sessions.length === 0 && <p className="text-xs text-[rgb(var(--text-faint))]">{t('village_no_sessions')}</p>}
          </CardContent>
        </Card>
        ...
        <Card className="bg-[rgb(var(--ink-card))] border-[rgb(var(--ink-border))]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-[rgb(var(--text-main))] flex items-center gap-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
              <HandHeart size={15} weight="duotone" className="text-[rgb(var(--gold))]" /> {t('village_circle')}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5">
            {circle.map((p, i) => (
              <button key={`${p.id}-${i}`} onClick={() => navigate(p.relationship === 'family' ? '/family' : '/learning-circles')}
                ...
                <span className="flex-1 min-w-0">
                  <span className="block text-xs text-[rgb(var(--text-main))] truncate">{p.name}</span>
                  <span className="block text-[10px] text-[rgb(var(--text-muted))]">{REL_LABELS[p.relationship] || p.relationship}{p.in_village ? '' : ' · beyond the village'}</span>
                </span>
              </button>
            ))}
            {circle.length === 0 && <p className="text-xs text-[rgb(var(--text-faint))]">{t('village_no_circle')}</p>}
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-[rgb(var(--ink-card))] border-[rgb(var(--gold)/0.25)]">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm text-[rgb(var(--text-main))] flex items-center gap-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            <Target size={15} weight="duotone" className="text-[rgb(var(--gold))]" /> {t('village_goals')}
            {goals.length > 0 && (
              <span className="text-[10px] text-[rgb(var(--text-muted))] font-normal">{goals_done}/{goals.length} carried home</span>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {goals.length > 0 ? (
            <>
              <div className="h-1.5 rounded-full bg-[rgb(var(--ink-deep))] border border-[rgb(var(--ink-border))] overflow-hidden mb-3">
                <div className="h-full bg-[rgb(var(--gold)/0.7)] transition-all" style={{ width: `${goals.length ? Math.round((goals_done / goals.length) * 100) : 0}%` }} />
              </div>
              <div className="space-y-1">
                {goals.map(g => (
                  <div key={g.id} className="flex items-center gap-2">
                    <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${g.done ? 'bg-emerald-500/20 border-emerald-500/50' : 'border-[rgb(var(--ink-border-strong))]'}`}>
            ...
            <p className="text-xs text-[rgb(var(--text-faint))]">{t('village_no_goals')}</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="bg-[rgb(var(--ink-card))] border-[rgb(var(--ink-border))]">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm text-[rgb(var(--text-main))] flex items-center gap-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            <Chats size={15} weight="duotone" className="text-[rgb(var(--gold))]" /> {t('village_voices')}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {posts.map(p => (
            <button key={p.id} onClick={() => navigate('/community')}
              className="w-full p-3 rounded bg-[rgb(var(--ink-deep))] border border-[rgb(var(--ink-border))] text-left hover:border-[rgb(var(--gold)/0.25)]">
              <div className="flex items-center gap-2 mb-1">
                <Avatar person={p} size="w-5 h-5" />
                <span className="text-[10px] text-[rgb(var(--text-muted))]">{p.author_name}</span>
                <span className="text-[9px] px-1.5 rounded bg-[rgb(var(--ink-card))] border border-[rgb(var(--ink-border))] text-[rgb(var(--text-faint))]">{p.category}</span>
              </div>
              <p className="text-xs text-[rgb(var(--text-main))] mb-0.5">{p.title}</p>
              <p className="text-[11px] text-[rgb(var(--text-muted))] line-clamp-2">{p.content}</p>
              <p className="text-[10px] text-[rgb(var(--text-faint))] mt-1">{p.reply_count} replies · {p.like_count} likes</p>
            </button>
          ))}
          {posts.length === 0 && <p className="text-xs text-[rgb(var(--text-faint))]">{t('village_no_posts')}</p>}
        </CardContent>
      </Card>
    </div>
  );
...
export function VillageHomeGate({ user }) {
  ...
  if (!data || enrollCount === null) return <p className="text-sm text-[rgb(var(--text-muted))]">Loading…</p>;
    ...
    <div className="space-y-6 animate-fade-in-up max-w-3xl" data-testid="village-picker">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl text-[rgb(var(--text-main))]" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{t('village_pick')}</h1>
        </div>
        <p className="text-xs text-[rgb(var(--text-muted))]">{t('village_pick_sub')}</p>
      </div>
