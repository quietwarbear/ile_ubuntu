...
def verify_webhook_auth(request: Request) -> bool:
        """Verify RevenueCat webhook via Authorization header."""
        if not REVENUECAT_WEBHOOK_SECRET:
                    return True  # Skip verification if no secret configured
    auth_header = request.headers.get("Authorization", "")
    return auth_header == REVENUECAT_WEBHOOK_SECRET
...
async def revenuecat_webhook(request: Request):
        ...
        if REVENUECAT_WEBHOOK_SECRET and not verify_webhook_auth(request):
